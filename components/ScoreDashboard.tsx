...
export const ScoreDashboard: React.FC = () => {
    const {
        analysisResult: result,
        resetAnalysis: onReset,
        uploadState
    } = useApp();

    if (!result) return null;

    const originalText = uploadState.textInput || `(Content from file: ${uploadState.file?.name})`;
    const assetType = uploadState.assetType;
                        ...
                        <div className="overflow-y-auto flex-1 p-2 space-y-2">
                            {result.issues.map((issue) => (
                                <div
                                    ...
                                    className={`p-4 rounded-lg cursor-pointer border transition-all ${selectedIssue?.id === issue.id
                                            ? 'bg-indigo-50 border-indigo-200 shadow-sm'
                                            : 'bg-white border-transparent hover:bg-slate-50 hover:border-slate-200'
                                        }`}
                                >
                                    <div className="flex justify-between items-start mb-1">
                                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${issue.severity === 'High' ? 'bg-red-100 text-red-700' :
                            ...
                            <div className="flex flex-col h-full">
                                <div className="p-6 border-b border-slate-100">
                                    <div className="flex justify-between items-start">
                                        ...
                                        <span className={`px-3 py-1 rounded-full text-sm font-bold ${selectedIssue.severity === 'High' ? 'bg-red-100 text-red-600' : 'bg-amber-100 text-amber-600'
                                            }`}>
                                    ...
                                    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                                        ...
                                        <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
                                            ...
                                            <button
                                                ...
                                                className={`px-5 py-2 rounded-lg font-semibold text-sm flex items-center gap-2 transition-all shadow-sm ${fixedIssues.has(selectedIssue.id)
